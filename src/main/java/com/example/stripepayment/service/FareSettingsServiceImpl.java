...
import com.example.stripepayment.repository.FareSettingsRepository;
...
public class FareSettingsServiceImpl implements FareSettingsService {
    ...
    @Override
        ...
        fareSettingsRepository.findTopByOrderByUpdatedAtDesc().ifPresentOrElse(                existingSettings -> {
        ...
        fareSettings.setDailyRate(fareSettingsDTO.getDailyRate());        fareSettings.setWeeklyRate(fareSettingsDTO.getWeeklyRate());
    ...
    @Override
    public FareSettings createFareSettings(FareSettingsDTO fareSettingsDTO) {        FareSettings fareSettings = new FareSettings();
        // Set the fare settings based on the DTO
        fareSettings.setHourlyRate(fareSettingsDTO.getHourlyRate());        fareSettings.setDailyRate(fareSettingsDTO.getDailyRate());
        fareSettings.setWeeklyRate(fareSettingsDTO.getWeeklyRate());        fareSettings.setMonthlyRate(fareSettingsDTO.getMonthlyRate());
        fareSettings.setCreatedAt(LocalDateTime.now());
        fareSettings.setUpdatedAt(LocalDateTime.now());
        return fareSettingsRepository.save(fareSettings);    }
}